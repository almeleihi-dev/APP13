...
let operationsDependencyCache: ReturnType<typeof buildOperationsDependencySnapshot> | undefined;

function buildOperationsDependencySnapshot() {
  return {
    ...
    release: validateRuntimeRelease().valid,
  };
}

export function collectOperationsDependencyValidation() {
  operationsDependencyCache ??= buildOperationsDependencySnapshot();
  return operationsDependencyCache;
}

export function validateOperationsModuleCoverage(): boolean {
  return OPERATIONS_MODULE_IDS.length === 16;
}
