export const APPROVAL_MODULE_IDS = [
  "need",
  "action",
  "contract",
  "chat",
...
export type ApprovalModuleId = (typeof APPROVAL_MODULE_IDS)[number];

export type ApprovalModuleStatus = "approved" | "conditional" | "pending" | "denied";

export interface ApprovalModuleOverview {
  id: ApprovalModuleId;
  label: string;
  delegateModule: string;
  status: ApprovalModuleStatus;
  approved: boolean;
}

export const APPROVAL_MODULE_META: Record<
  ApprovalModuleId,
...
export interface ApprovalOverview {
  ...
  overallStatus: ApprovalModuleStatus;
  modules: ApprovalModuleOverview[];
}

export function calculateApprovalPercentage(approvedCount: number, total: number): number {
  if (total <= 0) return 0;
  return Math.round((approvedCount / total) * 100);
}

export function buildApprovalOverview(modules: ApprovalModuleOverview[]): ApprovalOverview {
  ...
  let overallStatus: ApprovalModuleStatus = "approved";
  ...
  return {
    ...
    conditionalCount,
    deniedCount,
    approvalPercentage: calculateApprovalPercentage(approvedCount, modules.length),
    overallStatus,
    modules,
  };
}
