export const LAUNCH_CONTROL_MODULE_IDS = [
  "need",
  "action",
  "contract",
  "chat",
...
export type LaunchControlModuleId = (typeof LAUNCH_CONTROL_MODULE_IDS)[number];

export type LaunchControlModuleStatus = "cleared" | "conditional" | "pending" | "blocked";

export interface LaunchControlModuleOverview {
  id: LaunchControlModuleId;
  label: string;
  delegateModule: string;
  status: LaunchControlModuleStatus;
  cleared: boolean;
}

export const LAUNCH_CONTROL_MODULE_META: Record<
  LaunchControlModuleId,
...
export interface LaunchControlOverview {
  moduleCount: number;
  clearedCount: number;
  conditionalCount: number;
  blockedCount: number;
  launchClearancePercentage: number;
  overallStatus: LaunchControlModuleStatus;
  modules: LaunchControlModuleOverview[];
}

export function calculateLaunchClearancePercentage(clearedCount: number, total: number): number {
  if (total <= 0) return 0;
  return Math.round((clearedCount / total) * 100);
}

export function buildLaunchControlOverview(modules: LaunchControlModuleOverview[]): LaunchControlOverview {
  const clearedCount = modules.filter((m) => m.status === "cleared").length;
  ...
  let overallStatus: LaunchControlModuleStatus = "cleared";
  if (blockedCount > 0) overallStatus = "blocked";
  else if (conditionalCount > 0) overallStatus = "conditional";
  else if (clearedCount < modules.length) overallStatus = "pending";
  return {
    moduleCount: modules.length,
    clearedCount,
    conditionalCount,
    blockedCount,
    launchClearancePercentage: calculateLaunchClearancePercentage(clearedCount, modules.length),
    overallStatus,
    modules,
  };
}
