...
export interface RuntimeLaunchControlValidationResult {
  valid: boolean;
  errors: string[];
  warnings: string[];
  summary: string;
  checked: {
    ...
    launchControlCoverage: boolean;
    readOnlyGuarantees: boolean;
    noDuplicatedRuntimeLogic: boolean;
  };
}

export function validateRuntimeLaunchControl(): RuntimeLaunchControlValidationResult {
  const errors: string[] = [];
  ...
  if (NEED_LAYOUT.id !== "need-layout") {
    errors.push("Runtime launch control center must align with need-layout navigation");
  }

  const deps = collectLaunchControlDependencyValidation();

  if (!deps.need) errors.push("CH3-X5 not available for launch control center");
  if (!deps.action) errors.push("CH3-X6 not available for launch control center");
  if (!deps.contract) errors.push("CH3-X7 not available for launch control center");
  if (!deps.chat) errors.push("CH3-X8 not available for launch control center");
  if (!deps.timeline) errors.push("CH3-X9 not available for launch control center");
  if (!deps.notification) errors.push("CH3-X10 not available for launch control center");
  if (!deps.profile) errors.push("CH3-X11 not available for launch control center");
  ...
  if (LAUNCH_CONTROL_MODULE_IDS.length !== 23) {
    errors.push("Runtime launch control center must cover twenty-three modules");
  }

  if (!validateLaunchControlCheckCompleteness()) {
    errors.push("Launch control check completeness check failed");
  }

  if (!validateLaunchControlModuleCoverage()) {
    errors.push("Launch control module coverage check failed");
  }

  if (LAUNCH_CONTROL_FIXED_TIMESTAMP.length === 0) {
    errors.push("Missing deterministic launch control timestamp");
  }
  ...
  if (RUNTIME_LAUNCH_CONTROL_VERSION.length === 0) {
    errors.push("Missing runtime launch control version");
  }

  void LAUNCH_CONTROL_CHECK_IDS;
  ...
  return {
    ...
    warnings,
    summary: valid
      ? `Runtime launch control center validation passed (${LAUNCH_CONTROL_MODULE_IDS.length} modules, authoritative launch clearance)`
      : `Runtime launch control center validation failed with ${errors.length} error(s)`,
    checked: {
      moduleCount: LAUNCH_CONTROL_MODULE_IDS.length,
      checkCount: LAUNCH_CONTROL_CHECK_IDS.length,
      ...
      launchControlCoverage:
        validateLaunchControlModuleCoverage() && validateLaunchControlCheckCompleteness() && allDepsValid,
...
export { RUNTIME_LAUNCH_CONTROL_VERSION };
