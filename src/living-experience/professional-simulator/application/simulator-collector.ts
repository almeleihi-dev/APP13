...
import type { SimulatorEngineSnapshot } from "../domain/simulator-sections.js";
...
export interface LivingProfessionalSimulatorEngineDeps {
...
export function collectLivingProfessionalSimulatorEngineSnapshot(input: {
  authContext: AuthContext;
  context: LivingProfessionalSimulatorContext;
  engines: LivingProfessionalSimulatorEngineDeps;
  question?: string;
}): SimulatorEngineSnapshot {
  const { authContext, context: _context, engines, question } = input;
  const snapshot: SimulatorEngineSnapshot = {};
  ...
  try {
    const orchestration = engines.intelligenceOrchestration.recommend(authContext, {
      intent: question ?? "What professional what-if scenario should I explore today?",
    }) as {
      trust_score?: number;
