...
import type { AchievementsEngineSnapshot } from "../domain/achievements-sections.js";
...
export interface LivingProfessionalAchievementsEngineDeps {
...
export function collectLivingProfessionalAchievementsEngineSnapshot(input: {
  authContext: AuthContext;
  context: LivingProfessionalAchievementsContext;
  engines: LivingProfessionalAchievementsEngineDeps;
}): AchievementsEngineSnapshot {
  const { authContext, context: _context, engines } = input;
  const snapshot: AchievementsEngineSnapshot = {};
  ...
  try {
    const orchestration = engines.intelligenceOrchestration.recommend(authContext, {
      intent: "What professional achievements should be recognized?",
    }) as {
      trust_score?: number;
