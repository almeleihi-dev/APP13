...
import type { AnalyticsEngineSnapshot } from "../domain/analytics-sections.js";
...
export interface LivingProfessionalAnalyticsEngineDeps {
...
export function collectLivingProfessionalAnalyticsEngineSnapshot(input: {
  authContext: AuthContext;
  context: LivingProfessionalAnalyticsContext;
  engines: LivingProfessionalAnalyticsEngineDeps;
}): AnalyticsEngineSnapshot {
  const { authContext, context: _context, engines } = input;
  const snapshot: AnalyticsEngineSnapshot = {};
  ...
  try {
    const orchestration = engines.intelligenceOrchestration.recommend(authContext, {
      intent: "What professional analytics insights matter most today?",
    }) as {
      trust_score?: number;
