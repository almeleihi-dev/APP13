...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X39 regression suite"
...
echo "==> X38 operator experience integrity center regression"
npm run verify:x38

echo "==> X39 operator onboarding readiness center tests"
npm run test:x39-operator-onboarding-readiness
...
echo "X39 operator onboarding readiness center verification complete"
