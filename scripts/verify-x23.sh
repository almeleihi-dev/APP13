...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X23 regression suite"
...
echo "==> X22 executive experience layer regression"
bash scripts/verify-x22.sh

echo "==> X23 executive architecture review experience tests"
node --import tsx --test --test-concurrency=1 test/x23-architecture-review.test.ts
...
echo "X23 executive architecture review experience verification complete"
