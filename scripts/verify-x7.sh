...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X7 regression suite"
...
echo "==> X6 escrow payment regression"
bash scripts/verify-x6.sh

echo "==> X7 trust reputation experience tests"
node --import tsx --test --test-concurrency=1 test/x7-trust-reputation.test.ts
...
echo "X7 trust reputation experience verification complete"
