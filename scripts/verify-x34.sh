...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X34 regression suite"
...
echo "==> X33 browser entry and static delivery regression"
npm run verify:x33

echo "==> X34 browser hub and journey route tests"
npm run test:x34-browser-hub-routes
...
echo "X34 browser hub and journey routes verification complete"
