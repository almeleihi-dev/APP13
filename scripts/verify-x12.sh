...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X12 regression suite"
...
echo "==> X11 provider command center regression"
bash scripts/verify-x11.sh

echo "==> X12 customer command center experience tests"
node --import tsx --test --test-concurrency=1 test/x12-customer-command-center.test.ts
...
echo "X12 customer command center experience verification complete"
