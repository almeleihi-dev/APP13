...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for RC-1 regression suite"
...
echo "==> S3–S14 marketplace slice regression"
bash scripts/verify-s14.sh

echo "==> S15 release readiness analysis"
node --import tsx --test --test-concurrency=1 test/s15-release-readiness.test.ts

echo "==> Generating RC-1 reports"
npm run report:rc1

echo "APP13 RC-1 verification complete"
