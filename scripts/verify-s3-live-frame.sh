...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for live frame integration"
  ...
  echo "==> Docker not available — running unit-level live frame checks only"
fi
...
echo "S3 live frame verification complete"
