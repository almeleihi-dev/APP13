...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for financial safety integration"
  ...
  echo "==> Docker not available — running unit-level S3 checks only"
fi

echo "==> S3 foundation tests"
...
echo "S3 foundation verification complete"
