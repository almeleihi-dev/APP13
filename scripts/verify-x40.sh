...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X40 regression suite"
...
echo "==> X39 operator onboarding readiness center regression"
npm run verify:x39

echo "==> X40 action blueprint foundation engine tests"
npm run test:x40-action-blueprint
...
echo "X40 action blueprint foundation engine verification complete"
