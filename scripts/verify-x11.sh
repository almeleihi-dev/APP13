...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X11 regression suite"
...
echo "==> X10 live trust frame regression"
bash scripts/verify-x10.sh

echo "==> X11 provider command center experience tests"
node --import tsx --test --test-concurrency=1 test/x11-provider-command-center.test.ts
...
echo "X11 provider command center experience verification complete"
