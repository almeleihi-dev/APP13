...
async function main() {
  ...
  const tsOut = path.join(ROOT, "packages/tokens/src/tokens-data.ts");
  await writeFile(
    tsOut,
    `/** Auto-generated by scripts/sync-an-act-tokens.ts — do not edit */\nexport const TOKENS_PAYLOAD = ${JSON.stringify(payload, null, 2)} as const;\n`,
    "utf8"
  );
  console.log(`Synced tokens.json → ${OUT_DIR}`);
  console.log(`Synced tokens-data.ts → ${tsOut}`);
}
