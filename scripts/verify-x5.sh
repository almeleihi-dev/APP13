...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X5 regression suite"
...
echo "==> X4 action economy regression"
bash scripts/verify-x4.sh

echo "==> X5 request-to-match experience tests"
node --import tsx --test --test-concurrency=1 test/x5-request-match-experience.test.ts

echo "X5 request-to-match experience verification complete"
