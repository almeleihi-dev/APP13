...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X16 regression suite"
...
echo "==> X15 release readiness center regression"
bash scripts/verify-x15.sh

echo "==> X16 executive command center experience tests"
node --import tsx --test --test-concurrency=1 test/x16-executive-command-center.test.ts
...
echo "X16 executive command center experience verification complete"
