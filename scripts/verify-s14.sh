...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for S3–S14 regression suite"
...
echo "==> S8 match-to-contract conversion regression"
node --import tsx --test --test-concurrency=1 test/s8-match-contract-conversion.test.ts

echo "==> S9 customer order dashboard regression"
node --import tsx --test --test-concurrency=1 test/s9-customer-dashboard.test.ts

echo "==> S10 provider work dashboard regression"
node --import tsx --test --test-concurrency=1 test/s10-provider-dashboard.test.ts

echo "==> S11 operations admin console tests"
node --import tsx --test --test-concurrency=1 test/s11-admin-console.test.ts

echo "==> S12 notification event inbox tests"
node --import tsx --test --test-concurrency=1 test/s12-event-inbox.test.ts

echo "==> S13 platform search & discovery tests"
node --import tsx --test --test-concurrency=1 test/s13-discovery.test.ts

echo "==> S14 platform metrics & analytics tests"
node --import tsx --test --test-concurrency=1 test/s14-platform-analytics.test.ts
...
echo "S14 platform metrics & analytics verification complete"
