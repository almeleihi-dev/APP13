...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X8 regression suite"
...
echo "==> X7 trust reputation regression"
bash scripts/verify-x7.sh

echo "==> X8 discovery matching experience tests"
node --import tsx --test --test-concurrency=1 test/x8-discovery-matching.test.ts
...
echo "X8 discovery matching experience verification complete"
