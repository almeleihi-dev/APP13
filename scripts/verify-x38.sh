...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X38 regression suite"
...
echo "==> X37 operator surface navigation center regression"
npm run verify:x37

echo "==> X38 operator experience integrity center tests"
npm run test:x38-operator-experience-integrity
...
echo "X38 operator experience integrity center verification complete"
