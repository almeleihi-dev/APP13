...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X1 regression suite"
...
echo "==> X1 unified home experience tests"
node --import tsx --test --test-concurrency=1 test/x1-home-experience.test.ts

echo "X1 unified home experience verification complete"
