...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X37 regression suite"
...
echo "==> X36 browser experience completeness center regression"
npm run verify:x36

echo "==> X37 operator surface navigation center tests"
npm run test:x37-operator-surface-navigation
...
echo "X37 operator surface navigation center verification complete"
