...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X17 regression suite"
...
echo "==> X16 executive command center regression"
bash scripts/verify-x16.sh

echo "==> X17 launch simulation engine experience tests"
node --import tsx --test --test-concurrency=1 test/x17-launch-simulation.test.ts
...
echo "X17 launch simulation engine experience verification complete"
