...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X3 regression suite"
...
echo "==> X2 live frame regression"
bash scripts/verify-x2.sh

echo "==> X3 contract journey experience tests"
node --import tsx --test --test-concurrency=1 test/x3-contract-journey.test.ts

echo "X3 contract journey experience verification complete"
