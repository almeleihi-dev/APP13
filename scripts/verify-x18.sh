...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X18 regression suite"
...
echo "==> X17 launch simulation engine regression"
bash scripts/verify-x17.sh

echo "==> X18 investor readiness center experience tests"
node --import tsx --test --test-concurrency=1 test/x18-investor-readiness.test.ts
...
echo "X18 investor readiness center experience verification complete"
