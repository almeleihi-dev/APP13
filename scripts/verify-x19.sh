...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X19 regression suite"
...
echo "==> X18 investor readiness center regression"
bash scripts/verify-x18.sh

echo "==> X19 government partnership center experience tests"
node --import tsx --test --test-concurrency=1 test/x19-government-partnership.test.ts
...
echo "X19 government partnership center experience verification complete"
