...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for reputation timeline integration"
  ...
  echo "==> Docker not available — running unit-level timeline checks only"
fi
...
echo "==> S3 reputation timeline tests"
...
echo "S3 reputation timeline verification complete"
