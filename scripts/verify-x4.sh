...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X4 regression suite"
...
echo "==> X3 contract journey regression"
bash scripts/verify-x3.sh

echo "==> X4 action economy experience tests"
node --import tsx --test --test-concurrency=1 test/x4-action-economy.test.ts

echo "X4 action economy experience verification complete"
