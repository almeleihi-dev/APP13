...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X20 regression suite"
...
echo "==> X19 government partnership center regression"
bash scripts/verify-x19.sh

echo "==> X20 strategic operating system experience tests"
node --import tsx --test --test-concurrency=1 test/x20-strategic-operating-system.test.ts
...
echo "X20 strategic operating system experience verification complete"
