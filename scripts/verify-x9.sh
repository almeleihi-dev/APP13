...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X9 regression suite"
...
echo "==> X8 discovery matching regression"
bash scripts/verify-x8.sh

echo "==> X9 professional passport experience tests"
node --import tsx --test --test-concurrency=1 test/x9-professional-passport.test.ts
...
echo "X9 professional passport experience verification complete"
