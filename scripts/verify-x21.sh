...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X21 regression suite"
...
echo "==> X20 strategic operating system regression"
bash scripts/verify-x20.sh

echo "==> X21 mission control experience tests"
node --import tsx --test --test-concurrency=1 test/x21-mission-control.test.ts
...
echo "X21 mission control experience verification complete"
