...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X22 regression suite"
...
echo "==> X21 mission control regression"
bash scripts/verify-x21.sh

echo "==> X22 executive experience layer tests"
node --import tsx --test --test-concurrency=1 test/x22-executive-experience.test.ts
...
echo "X22 executive experience layer verification complete"
