...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X31 regression suite"
...
echo "==> X30 business intelligence center regression"
npm run verify:x30

echo "==> X31 executive UX readiness center experience tests"
npm run test:x31-executive-ux-readiness
...
echo "X31 executive UX readiness center experience verification complete"
