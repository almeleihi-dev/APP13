...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X32 regression suite"
...
echo "==> X31 executive UX readiness center regression"
npm run verify:x31

echo "==> X32 browser surface wiring tests"
npm run test:x32-browser-surface
...
echo "X32 browser surface wiring verification complete"
