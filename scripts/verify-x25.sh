...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X25 regression suite"
...
echo "==> X24 API surface audit center regression"
bash scripts/verify-x24.sh

echo "==> X25 production readiness center experience tests"
node --import tsx --test --test-concurrency=1 test/x25-production-readiness.test.ts
...
echo "X25 production readiness center experience verification complete"
