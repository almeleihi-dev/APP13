...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X6 regression suite"
...
echo "==> X5 request-to-match regression"
bash scripts/verify-x5.sh

echo "==> X6 escrow payment experience tests"
node --import tsx --test --test-concurrency=1 test/x6-escrow-payment-experience.test.ts

echo "X6 escrow payment experience verification complete"
