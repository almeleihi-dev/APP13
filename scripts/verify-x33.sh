...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X33 regression suite"
...
echo "==> X32 browser surface wiring regression"
npm run verify:x32

echo "==> X33 browser entry and static delivery tests"
npm run test:x33-browser-entry-static
...
echo "X33 browser entry and static delivery verification complete"
