...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X24 regression suite"
...
echo "==> X23 executive architecture review regression"
bash scripts/verify-x23.sh

echo "==> X24 API surface audit center experience tests"
node --import tsx --test --test-concurrency=1 test/x24-api-audit.test.ts
...
echo "X24 API surface audit center experience verification complete"
