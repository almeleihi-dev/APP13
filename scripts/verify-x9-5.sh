...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X9.5 regression suite"
...
echo "==> X9 professional passport regression"
bash scripts/verify-x9.sh

echo "==> X9.5 professional seals experience tests"
node --import tsx --test --test-concurrency=1 test/x9-5-professional-seals.test.ts
...
echo "X9.5 professional seals experience verification complete"
