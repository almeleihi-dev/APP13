...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for X2 regression suite"
...
echo "==> X1 unified home regression"
bash scripts/verify-x1.sh

echo "==> X2 live frame experience tests"
node --import tsx --test --test-concurrency=1 test/x2-live-frame-experience.test.ts

echo "X2 live frame experience verification complete"
