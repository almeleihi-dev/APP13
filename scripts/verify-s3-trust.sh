...
if [[ -n "$DOCKER" ]]; then
  echo "==> Starting PostgreSQL (Docker) for trust engine integration"
  ...
  echo "==> Docker not available — running unit-level trust checks only"
fi
...
echo "S3 trust engine verification complete"
